package com.huan.ted.message.components;
...
public class CaptchaConfig implements SystemConfigListener {
    ...
    public void updateLoginFailureInfo(Cookie  cookie ) {
        ...
        if (cookie != null) {
            String failureTimes = redisTemplate.opsForValue().get(cookie.getValue());
            if (failureTimes == null) {
                redisTemplate.opsForValue().set(cookie.getValue(), this.wrongTimes.toString(),expire,TimeUnit.SECONDS);
            } else {
                Integer times = Integer.parseInt(failureTimes);
                if (times < this.wrongTimes) {
                    redisTemplate.opsForValue().increment(cookie.getValue(), 1);
                }
            }
        }
    }
